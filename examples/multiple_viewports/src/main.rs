...
fn main() -> eframe::Result {
    ...
    eframe::run_native(
        ...
        options,
        Box::new(|cc| {
            Ok(Box::<MyApp>::new(MyApp::new(cc.egui_ctx.clone())))
        }),
    )
}
...
impl eframe::App for MyApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.deferred_viewport_close_requested.load(Ordering::Relaxed) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
...

            {
                ...
                egui::ComboBox::from_label("Background thread mode")
                    ...
                    .show_ui(ui, |ui| {
                        ui.selectable_value(background_thread_mode, BackgroundThreadMode::AutoRefresh, "AutoRefresh");
                        ui.selectable_value(background_thread_mode, BackgroundThreadMode::Pause, "Pause");
                    });
            }
            ...
            if let Some(immediate_viewport_id) = self.deferred_viewport_id {
                ui.group(|ui| {
                    ...
                    ui.label(format!("frame number: {}", ctx.cumulative_frame_nr_for(immediate_viewport_id)));
                });
            }
    ...
    fn on_exit(&mut self, _gl: Option<&eframe::glow::Context>) {
        *self.background_thread_mode.lock() = BackgroundThreadMode::Stop;
        let counter = self.background_thread_handle.take().unwrap().join().unwrap();
